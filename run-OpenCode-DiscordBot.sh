...
LOCKFILE="/tmp/opencode-OpenCode-DiscordBot.lock"
...
if [ "${1:-launch}" = "launch" ]; then
  ...
  if [ -f "$LOCKFILE" ]; then
    ...
    if [ -n "$LOCK_PID" ] && kill -0 "$LOCK_PID" 2>/dev/null && [ "${2:-}" != "continue" ]; then
      echo "OpenCode-DiscordBot is already running (PID $LOCK_PID)."
...
nohup bash "$BOX_DIR/scripts/auto-handoff.sh" --watch --name "OpenCode-DiscordBot" --data-dir "$DATA" --handoffs "$HANDOFFS_DIR" --pidfile "$DATA/opencode.pid" --workdir "$PROJECTS_DIR/$PROJ" --restart-cmd "$0 launch continue" >/dev/null 2>&1 &
